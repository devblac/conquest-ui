...
const LeftBoard = ({ cardPiles, gameState, handleAction }) => {
    ...
    <Grid container spacing={2} sx={{ justifyContent: 'center' }}>
      {treasureVictoryCardPiles.map((pile, index) => (
        <Grid item key={index} xs={4} sm={3} md={2} sx={{ display: 'flex', justifyContent: 'center' }}>
          <CardPile card={pile.card} count={pile.count} handleAction={resolveBuyAction(pile.card.id, gameState.youPlayerID)} />
        </Grid>
      ))}
    </Grid>
  );
